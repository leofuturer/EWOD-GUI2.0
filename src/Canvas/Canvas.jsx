...
export default function Canvas() {
  ...
  useEffect(() => {
    // mouseover eventlistener over whole canvas
    ...
    for (let i = 0; i < allCombined.length; i += 1) {
      ...
      if (
        ...
        if (isArrayInArray(byX[x], [y2, layVal])) {
          if (isArrayInArray(byX[x2], [y2, layVal])) {
            // has electrode on three sides
            pathstring = `M${x} ${y} L${x} ${y2} L${x2} ${y2} L${x2} ${y} Z `;
          } else {
            // has electrode on right and bottom side only
            pathstring = `M${x} ${y} L${x} ${y2 - 5} L${x2} ${
              y2 - 5
            } L${x2} ${y} Z `;
            pathstring += `M${x} ${y2 - 5} L${x} ${y2} L${x2 - 5} ${y2} L${
              x2 - 5
            } ${y2 - 5} Z `;
          }
        } else {
          // has electrode on right only
          pathstring = `M${x} ${y} L${x} ${y2 - 5} L${x2} ${
            y2 - 5
          } L${x2} ${y} Z `;
        }
      } else if (isArrayInArray(byX[x], [y2, layVal])) {
        // has electrode on down only
        pathstring = `M${x} ${y} L${x} ${y2} L${x2 - 5} ${y2} L${
          x2 - 5
        } ${y} Z `;
      } else {
        // has no electrode on either down or right
        pathstring = `M${x} ${y} L${x} ${y2 - 5} L${x2 - 5} ${y2 - 5} L${
          x2 - 5
        } ${y} Z `;
      }
      ...
      newSelectables.push({
        ...
        className: `electrode
          ${
            ...
            pinActuate.get(currentStep).content.has(elecToPin[`S${ids}`])
              ? "toSeq"
              : ""
          }
          ...
          ${
            mode === "PIN" &&
            (currElec === `S${ids}` || selected.includes(`${ids}`))
              ? "toPin"
              : ""
          }`,
    ...
    Object.entries(finalCombines).forEach((comb) => {
      newSelectables.push({
        ...
        className: `electrode
          ...
          ${
            mode === "CAN" && combSelected.includes(`${comb[0]}`)
              ? "selected"
              : ""
          }
          ${
            mode === "PIN" &&
            (currElec === `C${comb[0]}` || combSelected.includes(`${comb[0]}`))
              ? "toPin"
              : ""
          }`,
        scale: scaleXY.scale,
  ...
  function onSelectChange(selectedElecs) {
    ...
    if (mode === "PIN") {
      if (shiftDown) {
        ...
        if (newSelected[0].length + newSelected[1].length === 1) {
          // if selected one
          if (selected.length + combSelected.length > 1)
            setCurrElec(null); // if deselecting
          else if (newSelected[0].length)
            setCurrElec(`S${sIds[0]}`); // if selected square
          else setCurrElec(`C${cIds[0]}`); // if selected combined
        }
      } else {
        // not holding 'shift' down
        if (selectedElecs.length === 1) {
          if (sIds.length) setCurrElec(`S${sIds[0]}`); // if selected square
          else setCurrElec(`C${cIds[0]}`); // if selected combined
        } else {
          // selected multiple electrodes so has nothing to do with assigning pins
          setCurrElec(null);
        }
        setSelected(sIds);
    ...
    if (selectedElecs.length === 1) {
      if (mode === "SEQ") {
        if (
          sIds.length &&
          Object.prototype.hasOwnProperty.call(elecToPin, `S${sIds[0]}`)
        ) {
          handleActuationMapping(elecToPin[`S${sIds[0]}`]);
        } else if (
          cIds.length &&
          Object.prototype.hasOwnProperty.call(elecToPin, `C${cIds[0]}`)
        ) {
  ...
  function copy() {
    ...
    if (selected.length > 0) {
      const elements = electrodes.filter((element) =>
        selected.includes(`${element.ids}`)
      );
      elements.forEach((element) => {
        const tmp = [
          element.initPositions[0] + element.deltas[0],
  ...
  function handleCutFlag(
    ...
    if (numSquaresCopied > 0) {
      let maxID =
        electrodes.length === 0 ? 0 : electrodes[electrodes.length - 1].ids + 1;
      const tmps = [];
    ...
    if (numSquaresCopied > 0 || numCombinedCopied > 0) {
      ...
      if (numSquaresCopied > 0) {
        ...
        for (let i = 0; i < numSquaresCopied; i += 1) {
          const temp = [
            x + squares[i][0] - offsetX,
            y + squares[i][1] - offsetY,
          ];
          if (
            temp[0] < 0 ||
            temp[0] >= CANVAS_TRUE_WIDTH ||
            temp[1] < 0 ||
            temp[1] >= CANVAS_TRUE_HEIGHT
          ) {
          ...
          if (
            !(
              electrodes.some(
                (inner) =>
                  inner.initPositions[0] === temp[0] &&
                  inner.initPositions[1] === temp[1]
              ) ||
              allCombined.some(
                (inner) => inner[0] === temp[0] && inner[1] === temp[1]
              )
            )
          ) {
            newInits.push(temp);
      ...
      if (numCombinedCopied > 0) {
        const first =
          clipboard.squares.length > 0 ? clipboard.squares[0] : combined[0];
        const newCombs = [];
        ...
        for (let k = 0; k < numCombinedCopied; k += 1) {
          ...
          if (
            ...
            newCombs.push([temp[0], temp[1], combined[k][2] + maxID + 1]);
          } else {
  ...
  function squaresDelete() {
    ...
    electrodes.forEach((element) => {
      if (selected.includes(`${element.ids}`)) {
        ...
        if (mappedPin) {
          // mapping exists for this electrode so delete mapping
    ...
    const newElectrodes = electrodes.filter(
      (element) => !selected.includes(`${element.ids}`)
    );
    setSelected([]);
  ...
  function combinedDelete() {
    ...
    setElecToPin({ ...elecToPin });
    setComboLayout(
      allCombined.filter((combi) => !combSelected.includes(`${combi[2]}`))
    );
    setCombSelected([]);
  }
  ...
  function getCombinedLastFreeInd() {
    ...
    const newLastFreeInd = availIDs.find(
      (layoutVal) => !layVals.has(layoutVal)
    );
  ...
  function handleCombine(e) {
    ...
    connect(
      positions[0][1] / ELEC_SIZE - startY,
      positions[0][0] / ELEC_SIZE - startX
    );
  ...
  function separate() {
    ...
    const selectedCombs = allCombined.filter((x) =>
      combSelected.includes(`${x[2]}`)
    );
    let maxID =
      electrodes.length === 0 ? 0 : electrodes[electrodes.length - 1].ids + 1;
    const tmps = [];
    ...
    >
      {mode === "DRAW" || moving ? (
        <TransformWrapper
          ...
          panning={{
            disabled: !panning,
            excluded: ["react-transform-wrapper"],
          }}
                  ...
                  <path
                    ...
                    className={`electrode
                                    ${
                                      ...
                                      pinActuate
                                        .get(currentStep)
                                        .content.has(elecToPin[`C${comb[0]}`])
                                        ? "toSeq"
                                        : ""
                                    }
                                    ${
                                      mode === "CAN" &&
                                      combSelected.includes(`${comb[0]}`)
                                        ? "selected"
                                        : ""
                                    }
                                    ${
