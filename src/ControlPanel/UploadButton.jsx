        ...
        for (let i = 0; i < stringList.length; i += 1) {
          ...
          if (e !== '') {
            ...
            if (words.length >= 3 && words[0] === 'square' && !Number.isNaN(words[1]) && !Number.isNaN(words[2])) {
              const temp = {};
              temp.initPositions = [parseInt(words[1], 10) * ELEC_SIZE,
                parseInt(words[2], 10) * ELEC_SIZE];
              temp.deltas = [0, 0];
              temp.ids = i;
              ...
              newAllCombined.push([parseInt(words[1], 10) * ELEC_SIZE,
                parseInt(words[2], 10) * ELEC_SIZE, parseInt(words[3], 10)]);
              if (words.length > 4) {
